    ...
    public class MoveToMapMarker : ProfileBehavior
    {
        ...
        protected override Composite CreateBehavior()
        {
            ...
            new Sequence(
                ...
                new DecoratorContinue(ret => _actor == null && _miniMapMarker == null && Position == Vector3.Zero,
                    new Sequence(
                        new Action(ret => Logger.Log("Error: Could not find MiniMapMarker nor PortalObject nor Position {0}", Status())),
                        new Action(ret => _isDone = true)
                    )
                ),
                new Sequence(
        ...
        private void FindMiniMapMarker()
        {
            ...
            {
                Logger.Log("objective marker");

                _miniMapMarker = ZetaDia.Minimap.Markers.CurrentWorldMarkers.Where(m => m.IsPointOfInterest && m.Id < 1000).OrderBy(m => m.Position.Distance2D(ZetaDia.Me.Position)).FirstOrDefault();

                if (_miniMapMarker != null)
                {
                    Logger.Log("objective marker FOUND");
